...
var allowedTumourStatuses = ["ONSET", "POST-CHEMO", "RELAPSE", "POST-CHEMO RELAPSE"];
var allowedQualities = ["GOOD","AVERAGE","POOR","N.D."];
var request = BluebirdPromise.promisify(require("request"));
...
Migrator.prototype = {
    ...
    createPrimary: function(sample, idSubj, tumourStatus) {
        if (!sample.BIT_CODE) {
            console.log ("Migrator.migrateSampleDerivatives - sample misses required info: " + sample.ID_SAMPLE);
            return;
        }
        ...
        var metadata = {
            ...
            sample_codification: {value: sample.DESCR_BIT_TISSUE_NAME || 'ND'},
            pathology: {value: sample.DESCR_BIT_HISTOPATHOLOGY || 'ND'},
    ...
    migrateNucleicDerivatives: function(mysqlSampleId, idSubj) {
        ...
        .then(function(qcRows) {
            ...
            return BluebirdPromise.each(derivatives, function(derivative) {
                ...
                // console.log(qc);
                if (derivative.QUANTITY == null) {
                    console.log("Migrator.migrateSampleDerivatives - sample " + derivative.ID_SAMPLE + " has no quantity information");
                }
    ...
    migrateCGH: function(folder, ext) {
        ...
        return BluebirdPromise.map(files, function(file, index) {
            console.log(file);
            var metadataBatch = utils.composeCGHMetadata(file);
            console.log("Migrator.migrateCGH - here we are");
            // console.log(metadataBatch);
            var param = '{"arrival_code": {"value": "' + metadataBatch.sampleCode + '"}}';
            console.log(param);
        ...
        .catch(function(error) {
            console.log("Migrator.migrateCGH - error caught: " + error.message);
        });
    },
