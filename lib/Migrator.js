...
Migrator.prototype = {
    ...
    migrateNucleicDerivatives: function(mysqlSampleId, idSubj) {
        ...
        .leftJoin('SAMPLE_SCOLL', 'SAMPLE_SCOLL.ID_SAMPLE', 'SAMPLE.ID_SAMPLE')
        .leftJoin('SAMPLE_COLLECTION', 'SAMPLE_COLLECTION.ID_SCOLL', 'SAMPLE_SCOLL.ID_SCOLL')
        .leftJoin('DATA', 'DATA.ID_SCOLL', 'SAMPLE_COLLECTION.ID_SCOLL')
        .whereIn('ID_SAMPLE_TYPE', ['DNA','RNA']).andWhere('ID_PARENT_SAMPLE', mysqlSampleId)
        .andWhere('SAMPLE_COLLECTION.DESCR_SCOLL', 'SINGLE SAMPLE').whereIn('DATA.DESCR_DATA', ['QUALITY CONTROL - RNA', 'QUALITY CONTROL - DNA'])
        ...
        .then(function(rows) {
            ...
            return BluebirdPromise.each(rows, function(sample) {
                metadata = {
                    sampling_date: {value: sample.EXTRACTION_DATE},
                    quantity: {value: sample.QUANTITY, unit: 'µg'},
                    concentration: {value: sample.CONCENTRATION, unit: 'ng/µl'}
                };
                ...
                return that.knexPg.returning('id').insert({
                    'type': that.dataTypeMap[sample.ID_SAMPLE_TYPE],
                    'biobank': 1,
                    'biobank_code': sample.BIT_CODE,
                    'parent_subject': idSubj,
                    'parent_sample': that.sampleMap[mysqlSampleId],
