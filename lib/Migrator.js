...
const coroutines = {
    ...
    migrateCGH: BluebirdPromise.coroutine(function* (folder, ext, objInfoString) {

        let objInfo = JSON.parse(objInfoString);
        let notInserted = [], created = 0, data = [];

        let files = utils.getFilesInFolder(folder, ext);
        if (files.length === 0){
            logger.log('info', "Invalid or no files loaded");
            return BluebirdPromise.rejected({error:{message:"No Valid files loaded"}});
        }
        yield BluebirdPromise.each(files,co.wrap( function *(file) {
            ...
            if (!body.data[0]) {
                return BluebirdPromise.rejected("Migrator.migrateCGH: no sample found");
            }
            ...
            if (!bodySample) {
                return BluebirdPromise.rejected("Migrator.migrateCGH: no DNA found");
            }
            ...
            yield BluebirdPromise.each(metadataBatch.cnletr, co.wrap( function *(cnv) {
                ...
                if (res.statusCode !== CREATED) {
                    notInserted.push({error: body, data: cnv});
                }
                else {
                    created = created + 1;
                }
        ...
        return {created: created, notUpdated: notInserted.length};
    }),

    migrateVCF: BluebirdPromise.coroutine(function* (folder, ext, objInfoString) {
        ...
        let files = utils.getFilesInFolder(folder, ext);
        if (files.length === 0){
            logger.log('info', "No Valid files loaded");
...
// CERCO IL SOGGETTO
            ...
            if (resSubjects.statusCode !== OK || !bodySubjects) {
                ...
                return BluebirdPromise.rejected("Server Error Getting Subject with code" + codePatient + ":");
            }
...
// CONTROLLO CHE IL FILE NON SIA GIÀ STATO INSERITO
            ...
            if (alreadyInserted) {
                logger.log('info', fileName + "was already inserted");
                return BluebirdPromise.rejected(fileName + "was already inserted:");
            }
...
// CERCO IL TESSUTO
            ...
            if (resTissues.statusCode !== OK || !bodyTissues) {
                ...
                return BluebirdPromise.rejected("Error Getting Tissue");
            }
//SE IL TESSUTO NON ESISTE LO CREO
            if (!bodyTissues.data[0]) {
                ...
                if (resTissueCr.statusCode !== CREATED) {
                    ...
                    return BluebirdPromise.rejected(resTissueCr.body);
...
//CREO I VCF METADATA
            let dataFile = yield new Promise(function (resolve, reject) {
                ...
                vcf.on('data', function (row){
                    if(row.sampleinfo[0] && subject && subject.id){
                        ...
                        let json = {
                            ...
                            metadata: metadataVcf,
                            parentData: analysis.id,
                            parentSubject: subject.id
                        };
                        rowsFilesReady.push(json);
                    }
                    else {
...
// CREO TUTTE LE VARIANTI
            yield BluebirdPromise.each(dataFile, co.wrap( function *(datum) {
                ...
                if (res.statusCode !== CREATED) {
                    notInserted.push({error: body, data: datum});
                }
                else {
                    created = created + 1;
                }
...
Migrator.prototype = {
    ...
    migrateCGH: function(folder, ext, objInfo) {

        return coroutines.migrateCGH(folder, ext, objInfo)
        .catch(function(err) {
            throw err;
        });
    },
     ...
     */
    migrateVCF: function(folder, ext, objInfo) {

        return coroutines.migrateVCF(folder, ext, objInfo)
        .catch(function(err) {
            throw err;
        });
    },
