...
const coroutines = {
                ...
                let [resCreaSubj, bodyCreaSubj] = yield request.postAsync({
                    ...
                    json: {
                        ...
                        metadata: {
                            ...
                            "iit_shipment_date": {
                              "group": "Details",
                              "value": patient['IIT Shipment Date'] ? moment(patient['IIT Shipment Date'], "DD-MM-YYYY").format("YYYY-MM-DD") : null,
                            },
                            "laboratory_id": {
    ...
    GetInsertAnalysisNGSforPT: BluebirdPromise.coroutine(function* (patient, daemons) {
            ...
            if (BodyAnalysis && _.isArray(BodyAnalysis) && BodyAnalysis.length > 0) {
                patient.idAnalysis = BodyAnalysis[0].id;
            }
                   ...
                   let [resCreaAn, bodyCreaAn] = yield request.postAsync({
                       ...
                       json: {
                           ...
                           metadata: {
                               "target": {
                                 "group": "Details",
                                 "value": patient['Seq Type'] ? patient['Seq Type'].toUpperCase() === 'WGS' ? 'WHOLE GENOME' : patient['Seq Type'].toUpperCase() === 'WES' ? "EXOME" : patient['Seq Type'].toUpperCase() === 'PANEL' ? "PANEL" : null : null
                               },
                            //    "platform": {
                            //      "group": "Details",
     ...
     */
    migrateNGSPATIENTS: BluebirdPromise.coroutine(function* (folder, ext) {
        var that = this;
        let files = utils.getFilesInFolder(folder, ext);
        ...
        let workbook = xlsx.readFile(files[0]);
        let worksheet = workbook.Sheets['Data Import'];
        let range = xlsx.utils.decode_range(worksheet['!ref']);
        ...
        // TODO
        // STEP :1 LEGGO LE RIGHE DALL EXCEL
        // STEP :2 PER OGNI RIGA CONTROLLO CHE ESISTA IL PAZIENTE: SE NON ESISTE CREO SE NO PRENDO ID
        // STEP :3 CONTROLLO CHE NON ESISTA IL CAMPIONE: SE NON ESISTE CREO SE NO PRENDO ID
        // STEP :4 SE RIGA HA CAMPI SEQ TYPE E/O KIT CREO ANALISI

       

    const grouped_results = _.mapValues(_.groupBy(patients, 'Family ID'), rlist => rlist.map(res => _.omit(res, 'Family ID')));
    ...
    yield BluebirdPromise.each(patients, co.wrap(function* (patient, index) {
        ...
        if (!patient['Family ID']) {
            ...
            let daemon = yield DaemonService.InitializeDeamon(" ", {}, 7, process, connections.bearerToken);
            ...
            return BluebirdPromise.rejected(errorString);
                ...
                let dmn = yield DaemonService.InitializeDeamon(patient['SAMPLE ID (LAB)'], {}, 7, process, connections.bearerToken);
                if (dmn && (dmn.status === 'initializing' || dmn.status === 'error')) {
                    // rowsTobeProcessed.push(datum['FLUIDO']);
        ...
        yield BluebirdPromise.each(families, co.wrap(function* (familyID) {
            ...
            // let probands =  _.filter(family, (f)=> f['Relationship'] === 'PROBAND' || f['Relationship'] === 'CONTROL');
            ...
            yield BluebirdPromise.each(probands, co.wrap(function* (proband) {
                ...
                if (!relative['Seq Type']) { //&& !relative['Platform'] && !relative['KIT']
   ...
   UpsertAnalysisNGSforAN: BluebirdPromise.coroutine(function* (analysis, metadata, daemon) {
           ...
           if (BodyAnalysis && _.isArray(BodyAnalysis) && BodyAnalysis.length == 1 ) {
               ...
               let [resUpdAn, bodyUpdAn] = yield request.putAsync({
                ...
                if (resUpdAn.statusCode !== CREATED) {
     ...
     * @description coroutine for import NGS Patients
     */
    createNGSANALYSIS: BluebirdPromise.coroutine(function* (folder, ext, processInfo) {
        var that = this;
        let objInfo = JSON.parse(processInfo.argv[2]);

        const rewritePaths = objInfo.rewritePaths;
        ...
        const samplesIDs = _.uniq(_.map(analysesDataSource, 'sample'));


    // yield BluebirdPromise.each(analysesDataSource, co.wrap(function* (an, index) {

    //     if (!an['sample'] && an['XTENS ID']) {
    //         let errorString = 'No ID provided, row: ' + index;
    //         let daemon = yield DaemonService.InitializeDeamon(" ", {}, 7, process, connections.bearerToken);
    //         daemon = yield DaemonService.ErrorDeamon(daemon, errorString, connections.bearerToken);
    //         logger.log('info', errorString);
    //         return BluebirdPromise.rejected(errorString);
    //     } 
    //     else if (!daemons[an['sample']]){
    //         let dmn = yield DaemonService.InitializeDeamon(an['sample'], {}, 7, process, connections.bearerToken);
    //         if (dmn && (dmn.status === 'initializing' || dmn.status === 'error')) {
    //             // rowsTobeProcessed.push(datum['sample']);
    //             daemons[an['sample']] = dmn;
                
    //             // daemons[an['sample']] = {
    //             //     'createdAt': '2020-10-09T11:48:07.000Z',
    //             //     'id' : 1070,
    //             //     'info' : {error: '', totalRows: 0, percentage: 0, processedRows: 0, notProcessedRows: Array(0)},
    //             //     'operator' : 7,
    //             //     'pid' : 84537,
    //             //     'source' : an['sample'],
    //             //     'status' : 'initializing',
    //             //     'updatedAt' : '2020-10-09T11:48:07.000Z'
    //             // };
    //         }
    //     }
    // }));
        ...
        yield BluebirdPromise.each(samplesIDs, co.wrap(function* (sampleID, index) {
            ...
            let daemon = yield DaemonService.InitializeDeamon(sampleID, {}, 7, process, connections.bearerToken);
            ...
            //TODO CERCO PAZIENTE E CAMPIONE
            ...
            if(!resAnalysis.analyses) return;
            analyses = resAnalysis.analyses;
