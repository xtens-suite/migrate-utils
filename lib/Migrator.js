...
Migrator.prototype = {
    ...
    migrateSamples: function(mysqlSubjId) {
        ...
        .then(function(rows) {
            ...
            return that.knexMysql.select('JSON_SCHEMA').from('DATA')
                ...
                .whereIn('DESCR_DATA', 'TUMOUR STATUS')
                .whereIn('ID_SAMPLE', primaryIds);
        })
        ...
        .then(function(tsRows) {
            ...
            return BluebirdPromise.each(tsRows, function(tsDatum, index) {
                return that.createPrimary(samples[index], pgSubjId, tsDatum);    
            });
    ...
    createPrimary: function(sample, idSubj, tumourStatus) {
        ...
        var that = this;
        var knexPg = this.knexPg;
        var sampleType, sampleTypeName, idPrimarySample;
        ...
        if (_.isEmpty(tumourStatus)) {
            ...
            metadata.tumour_status = {value: jsonSchema.body[0].content[0].instances[0]};
        }
    ...
    migrateNucleicDerivatives: function(mysqlSampleId, idSubj) {
        ...
        return this.knexMysql.select('SAMPLE.ID_SAMPLE','BIT_CODE', 'ID_SAMPLE_TYPE', 'QUANTITY', 'EXTRACTION_DATE', 'CONCENTRATION').from('SAMPLE')
        ...
        .then(function(qcRows) {
            ...
            console.log(qcRows);
            return BluebirdPromise.each(qcRows, function(qc, index) {
                console.log("Migrator.migrateNucleicDerivatives - qc: ");
                console.log(qc);

                metadata = {
                    sampling_date: {value: derivatives[index].EXTRACTION_DATE},
                    quantity: {value: derivatives[index].QUANTITY, unit: 'µg'},
                    concentration: {value: derivatives[index].CONCENTRATION, unit: 'ng/µl'}
                };
                ...
                if (!_.isEmpty(qc)) {
                    ...
                    metadata["260/280"] = {value: jsonSchema.body[0].content[2].instances[0].value};
                    metadata["260/230"] = {value: jsonSchema.body[0].content[3].instances[0].value};
                }
                ...
                return that.knexPg.returning('id').insert({
                    'type': that.dataTypeMap[derivatives[index].ID_SAMPLE_TYPE],
                    'biobank': 1,
                    'biobank_code': derivatives[index].BIT_CODE,
                    'parent_subject': idSubj,
                    'parent_sample': that.sampleMap[mysqlSampleId],
                    'metadata': metadata,
                    'created_at': new Date(),
