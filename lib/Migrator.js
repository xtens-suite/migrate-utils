...
const coroutines = {
    ...
    getVariantNGSFromXTENS: BluebirdPromise.coroutine(function * (variant, daemon, knex) {
        let sqlquery = `SELECT id FROM data WHERE type = 216 
    ...
    getPersInfoVariantNGSFromXTENS: BluebirdPromise.coroutine(function * (sample, variant, daemon, knex) {
        ...
        INNER JOIN (SELECT id FROM data WHERE type = 216 AND metadata->'chr'->>'value' = '${variant.chr}' AND metadata->'pos'->>'value' = '${variant.pos}' AND metadata->'alt'->>'value' = '${variant.alt}' AND metadata->'ref'->>'value' = '${variant.ref}' ) AS variant ON dtdt_3."data_parentData" = variant.id 
    ...
    migrateMasterNGSVCF: async function (folder, ext, processInfo, knex, crudManager) {
        ...
        var countriga = 1;
        await (async function processLineByLine () {
            try {
                const rl = readline.createInterface({
                    input: fs.createReadStream(files[0]).pipe(zlib.createGunzip()),
                    crlfDelay: Infinity
                });
                rl.on('line', async (rawLine, index) => {
                    ...
                    if (variantMasterFile) {
                        ...
                        // logger.log("info", "DO IMPORT :" + filteredSamples.length);
                            ...
                            // var variantXTENS = await that.getVariantFromXTENS(variantMasterFile.chr, variantMasterFile.pos, variantMasterFile.ref, variantMasterFile.alt, 216, daemon, knex);
...
function Migrator (mysqlConn, pgConn, annoPgConn) {
    ...
    this.knexPgAsync = BluebirdPromise.promisifyAll(require('knex')(connections[pgConn]));
    ...
    this.crudManager = new databaseManager.CrudManager(null, connections[pgConn].connection, connections.fileSystemConnections[connections.fileSystemConnections.default]);
}
