...
const coroutines = {
    ...
    getPersInfoVariantNGSFromXTENS: BluebirdPromise.coroutine(function * (filteredSamples, variant, knex) {
                        ...
                        await knex.transaction(async trx => {
                            ...
                            if (parseInt(variantMasterFile.varinfo.AC) <= AlleleCountMaxValue) {
                                ...
                                if (filteredSamples.length > 0) {
                                    // var start = new Date();
                                    var results = await that.getPersInfoVariantNGSFromXTENS(filteredSamples, variantMasterFile, trx);
                                    ...
                                    for await (const sample of filteredSamples) {
                                        ...
                                        if (!personalInfoID) {
                                            ...
                                            var analysisID = await that.getAnalysesNGSFromXTENS(res && res.biobank_code ? res.biobank_code : sample.NAME, daemon, trx);
                                    ...
                                    if (VariantSupportingInfoArray.length > 0) {
                                        ...
                                        var results1 = await knex('data').returning('id')
                                            ...
                                            .transacting(trx);
                                        var joinData2 = results1.map((d, i) => {
                            ...
                            if (!variantXTENS.id) {
                                variantXTENS.id = await that.getVariantNGSFromXTENS(variantMasterFile, daemon, trx);
                            }
                            ...
                            if (variantXTENS.id) {
                                ...
                                var resUpdate = await knex('data').returning('id')
                                    ...
                                    .update(variantXTENS)
                                    .transacting(trx);
                                var joinData1 = parentData.map((p) => {
