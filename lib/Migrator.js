...
const coroutines = {
    ...
    getVariantFromXTENS: BluebirdPromise.coroutine(function * (chr, pos, ref, alt, dtId, daemon) {
        let queryPayload = {
            "queryArgs": {
                "leafSearch": false,
                "wantsSubject": false,
    ...
    migrateMasterNGSVCF: BluebirdPromise.coroutine(async function * (folder, ext, processInfo) {
        var that = this;
        let objInfo = JSON.parse(processInfo.argv[2]);
        ...
        if (files.length !== 1) {
            let errorString = "Invalid or no files loaded";
            daemon = yield DaemonService.InitializeDeamon(" ", {}, 7, processInfo, connections.bearerToken);
            daemon = yield DaemonService.ErrorDeamon(daemon, errorString, objInfo.bearerToken);
            logger.log('info', "Invalid or no files loaded");
            return BluebirdPromise.rejected(errorString);
        }

        daemon = yield DaemonService.InitializeDeamon(files[0], {}, 7, processInfo, connections.bearerToken);

        // await getStreamLines(stream);

        // async function getStreamLines (readable) {
        // }
        ...
        //     //     var variantXTENS = await that.getVariantFromXTENS(variantMasterFile.chr, variantMasterFile.pos, variantMasterFile.ref, variantMasterFile.alt, 216);
        //     if (true) {
        //         // DO UPDATE
        //         variantXTENS = await that.InsertVariantFromXTENS(variantMasterFile.chr, variantMasterFile.pos, variantMasterFile.ref, variantMasterFile.alt, 216);
        //         updated = updated + 1;
        //         console.log(updated);
        //     } else {
        //         created = created + 1;
        //         console.log(created);
        //         // DO CREATE
        //         // let metadata = utils.composeVCFMetadata(variantMasterFile);
        //         // let json = {
        //         //     owner: parseInt(objInfo.owner),
        //         //     metadata: metadata,
        //         //     parentData: [analysis.id],
...
Migrator.prototype = {
    ...
    migrateMasterNGSVCF: function (folder, ext, process) {
        // let knexAnno = this.knexAnnoPg;
        return coroutines.migrateMasterNGSVCF(folder, ext, process)
            .catch(function (err) {
                throw err;
            });
    },
