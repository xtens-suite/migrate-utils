...
Migrator.prototype = {
    ...
    migrateCGH: function(folder, ext) {
        let that = this, idCghProcessed, idSubj, idSample;
        ...
        return BluebirdPromise.each(files, function(file, index) {
            ...
            let metadataBatch = utils.composeCGHMetadata("/home/gupta/Desktop/CGH/CARICATI/16-H-00492.xlsx");
            ...
            request.postAsync({
              ...
              .spread(function(res, body) {
                  ...
                  .spread(function(res, body) {
                      ...
                      return request.getAsync({
                          uri: basePath + '/data?type=6&parentSubject=' + idSubj+'&parentSample='+idSample,
                          auth: {
                              bearer: connections.bearerToken
                          }
                      });
                      ...
                      .spread(function(res, body) {
                          ...
                          if(!_.isEmpty(body)){
                              logger.log('info', 'CGH Raw found: ' + body) ;
                              
                              return request.getAsync({
                                  uri: basePath + '/data?type=7&parentSubject=' + idSubj+'&parentData='+body[0].id,
                                  auth: {
                                      bearer: connections.bearerToken
                                  }
                              });
                  ...
                  .spread(function(res, body) {
                      ...
                      if (res.statusCode !== CREATED && res.statusCode !== OK) {
                      ...
                      if(res.statusCode === OK){
                          body = JSON.parse(body);
                          if(!_.isEmpty(body)){
                              logger.log('info', 'CGH Processed found: ' + body) ;
                              idCghProcessed = body[0].id;
                              return request.getAsync({
                                  uri: basePath + '/data?type=18&parentSubject=' + idSubj+'&parentData='+idCghProcessed,
                                  auth: {
                                      bearer: connections.bearerToken
                                  }
                              });
                  ...
                  .spread(function(res, body) {
                      ...
                      if(res.statusCode === OK){
                          body = JSON.parse(body);
                          if(!_.isEmpty(body)){
                              logger.log('info', 'Genomic Profile found: ' + body) ;

                              return request.getAsync({
                                  uri: basePath + '/data?type=8&parentSubject=' + idSubj+'&parentData='+idCghProcessed,
                                  auth: {
                                      bearer: connections.bearerToken
                                  }
                              });
                      ...
                      logger.log('info', 'Created CGH-Processed: ' + body);
                    ...
                    .spread(function(res, body) {
                        ...
                        if(res.statusCode === OK){
                            body = JSON.parse(body);
                            if(!_.isEmpty(body)){
                                logger.log('info', 'CNV data found: ' + body) ;
    ...
    importCNBInfo: function(folder, ext) {
        ...
        .spread(function(res, body) {
            ...
            CNBInfos = body && body.data;

            return BluebirdPromise.each(patients, patient => {

                let CNBInfo = _.find(CNBInfos, function( obj ) {
                ...
                if(CNBInfo){
                    ...
                    if(subject){
                        ...
                        created = created + 1;
                        logger.log('info', "Migrator.importCNBInfo - patient: " + patient.Nome + " " + patient.Cognome + " CNBInfo to be created. " + created);
                        ...
                        logger.log('info',"Migrator.importCNBInfo - patient: " + patient.Nome + " " + patient.Cognome + " with code: " + patient.Cod_RINB + "is not present into DB. ");
                        return;
                    }
                }
            ...
            logger.log('info', "Migrator.importCNBInfo - Got some errors while updating CNBInfo:" + error.message);
            return false;
        });
        })
        .then(function() {
            logger.log('info', "Migrator.importCNBInfo - All went fine!");
            return true;
        })
        .catch(function(error) {
            logger.log('info', "Migrator.importCNBInfo - Got some errors while updating CNBInfo:" + error.message);
