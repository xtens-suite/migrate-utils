...
Migrator.prototype = {
    ...
    /**
     * @method
     * @name migrateCGHRecord
     * @description migrates a CGH data from an Excel file to the PostgreSQL database
     * @param{string} - filePath: the file path
     */
    migrateCGHRecord: function(filePath) {
        var workbook = xlsx.readFile(filePath);
        var worksheet = workbook.Sheets[workbook.SheetNames];
        var csvString = xlsx.utils.sheet_to_csv(worksheet);
        // console.log(csvString);
        var csvParser = parseCSV(csvString, {}, function(err, output) {
            if (err) {
                console.log("Error");
                console.log(err);
                return false;
            ...
            console.log("Done");
            console.log("output[0] is: " + output[0]);
            console.log("output[20] is: " + output[20]);
            /*
            for (var i=11; i<20; i++) {
