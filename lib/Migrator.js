...
const coroutines = {
    ...
    getVariantFromXTENS: BluebirdPromise.coroutine(function * (chr, pos, ref, alt, dtId, daemon, knex, trx) {
    // async function (chr, pos, ref, alt, dtId, daemon) {
        ...
        // var result = yield knex.raw(sqlquery).transacting(trx);
    ...
    getAnalysesNGSFromXTENS: BluebirdPromise.coroutine(function * (biobankCode, daemon, knex, trx) {
    ...
    getVariantNGSFromXTENS: BluebirdPromise.coroutine(function * (variant, daemon, knex, trx) {
    ...
    getPersInfoVariantNGSFromXTENS: BluebirdPromise.coroutine(function * (filteredSamples, variant, knex, trx) {
                    ...
                    // var start = new Date();
                        ...
                        // logger.log("info", countriga + " " + variantMasterFile.chr + " " + variantMasterFile.pos + " " + variantMasterFile.ref + " " + variantMasterFile.alt + " - count AC: " + variantMasterFile.varinfo.AC);
                            ...
                            if (filteredSamples > 0) {
                                // var start = new Date();
                                const results = await that.getPersInfoVariantNGSFromXTENS(filteredSamples, variantMasterFile, knex); //, trx);
                                ...
                                for await (const sample of filteredSamples) {
                                    ...
                                    if (!personalInfoID) {
                                        ...
                                        var analysisID = await that.getAnalysesNGSFromXTENS(res && res.biobank_code ? res.biobank_code : sample.NAME, daemon, knex); //, trx);
                                ...
                                // logger.log("info", "ROW: " + countriga + " - FilteredSamples Management duration in ms:" + duration);

                                if (VariantSupportingInfoArray.length > 0) {
                                    var parentAnalyses = _.flatten(VariantSupportingInfoArray.map(d => d.parentData));
                                    VariantSupportingInfoArray = VariantSupportingInfoArray.map(({ parentData, ...json }) => json);
                                    try {
                                        await knex.transaction(async trx => {
                                            var resultsJoin = await knex('data').returning('id')
                                                .insert(VariantSupportingInfoArray)
                                                .transacting(trx);
                                            var joinData1 = resultsJoin.map((d, i) => {
                            ...
                            if (!variantXTENS.id) {
                                variantXTENS.id = await that.getVariantNGSFromXTENS(variantMasterFile, daemon, knex); //, trx);
                            }
                                ...
                                try {
                                    await knex.transaction(async trx => {
                                        var resUpdate = await knex('data').returning('id')
                                            ...
                                            .update(variantXTENS)
                                            .transacting(trx);
                                        var joinData2 = parentData.map((p) => {
                                            return { "data_parentData": resUpdate[0], "data_childrenData": p };
                                        });
                                        if (joinData2.length > 0) {
                                            await knex('data_childrendata__data_parentdata')
                                                .insert(joinData2).onConflict(["data_parentData", "data_childrenData"])
                                                .ignore()
                                                .transacting(trx);
                                        }
                                        await trx.commit();
