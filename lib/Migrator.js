...
Migrator.prototype = {
    ...
    migrateNucleicDerivatives: function(mysqlSampleId, idSubj) {
        ...
        .then(function(qcRows) {
            ...
            return BluebirdPromise.each(derivatives, function(derivative) {
                ...
                /*
                   return that.knexMysql.select('URI').from('FILE_XTENS')
                   .where('ID_FCOLL', qc.ID_FCOLL).andWhere('FILE_TYPE', "DATA")

                   .then(function(files) {
                   if (!_.isEmpty(files)) {
                   logger.log('info', "Migrator.migrateSampleDerivatives - found files");
                   logger.log('info', files);
                   payload.files = _.map (files, file => {
                ...
                .spread(function(res, body) {
                    if (res.statusCode !== CREATED) {
                        ...
                        console.log('debug', res && res.request && res.request.body);
    ...
    migrateCGH: function(folder, ext) {
        ...
        return BluebirdPromise.each(files, function(file, index) {
            ...
            let param = '{"arrival_code": {"value": "' + metadataBatch.sampleCode + '"}}';
            ...
            return that.knexPg.raw(["SELECT s.id, s.metadata FROM sample s LEFT JOIN data_type d ON d.id = s.type", 
                                   ...
                                   .then(function(resp) {
                                       ...
                                       if (!_.isArray(resp.rows) || _.isEmpty(resp.rows)) {
                                           console.log("Migrator.migrateCGH: no sample found");
                                           return;
                                       }
                                       let idSample = resp.rows[0].id;
                                       let idSubj = resp.rows[0].donor;
                                       ...
                                       .spread(function(res, body) {
                                           // console.log(metadataBatch.acghProcessed);
                                           return request.postAsync({
                                               uri: basePath + '/data',
                                               auth: {
                                                   bearer: connections.bearerToken
                                       ...
                                       .spread(function(res, body) {
                                           return BluebirdPromise.each(metadataBatch.cnvArr, function(cnv) {
                                               return request.postAsync({
                                                   uri: basePath + '/data',
                                                   auth: {
                                                       bearer: connections.bearerToken
                                       ...
                                       .then(function(res) {
                                           console.log("Migrator.migrateCGH -  Done!");
                                       })
                                       .catch(function(err) {
                                           console.log("Migrator.migrateCGH - Exception caught while creating CGH data: " + err.message);
                                           // TODO Rollback
                                       }); 
                                   });
