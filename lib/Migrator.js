...
const r_module = require("request");
const r = r_module.defaults({'proxy':false})
const request = BluebirdPromise.promisify(r);
...
const coroutines = {
    ...
    getVariantFromXTENS: BluebirdPromise.coroutine(function * (chr, pos, ref, alt, dtId, daemon, knex, client) {
    // async function (chr, pos, ref, alt, dtId, daemon) {
        const sqlquery = "SELECT DISTINCT d.id, d.type, d.owner, d.metadata FROM data d WHERE d.type = " + dtId +
        ...
        const result = yield client.query(sqlquery);
    ...
    getAnalysesNGSFromXTENS: BluebirdPromise.coroutine(function * (biobankCode, daemon, knex, client) {
    ...
    getVariantNGSFromXTENS: BluebirdPromise.coroutine(function * (variant, daemon, knex, client) {
        const sqlquery = `SELECT id FROM data WHERE type = ${VariantDataTypeMasterVcfImport} 
    ...
    getPersInfoVariantNGSFromXTENS: BluebirdPromise.coroutine(function * (filteredSamples, variant, knex, client) {
        let sqlfirstPart = `select suppInfo.suppInfoID, variant.id as idVariant, suppInfo.biobank_code from (`;

        let sqlsecondPart = filteredSamples.map(sample => {
            return `SELECT persInfoVar.id as suppInfoID, d.biobank_code
            FROM sample d    
            JOIN data_parentsample__sample_childrendata AS dtsm_1 ON dtsm_1."sample_childrenData" = d.id       
            JOIN (SELECT id, metadata FROM data WHERE type = 212 AND metadata->'target'->>'value' = 'EXOME') AS analysis ON dtsm_1."data_parentSample" = analysis.id    
            JOIN data_childrendata__data_parentdata AS dtdt_2 ON dtdt_2."data_childrenData" = analysis.id                    
            JOIN (SELECT id, metadata FROM data WHERE type = 215 AND metadata->'ad'->>'value' = '${sample.AD}' AND metadata->'dp'->>'value' = '${sample.DP}' AND metadata->'gt'->>'value' = '${sample.GT}' AND metadata->'pl'->>'value' = '${sample.PL}' ) AS persInfoVar ON dtdt_2."data_parentData" = persInfoVar.id
            WHERE d.type = 211 AND d.biobank_code = '${sample.NAME}'
            `.replace(/(?:\r\n|\r|\n)/g, '');
        }).join(`
        UNION ALL
        `).replace(/(?:\r\n|\r|\n)/g, '');
        let sqlthirdPart = `) as suppInfo
        JOIN data_childrendata__data_parentdata AS dtdt_3 ON dtdt_3."data_childrenData" = suppInfo.suppInfoID     
        JOIN (SELECT id FROM data WHERE type = ${VariantDataTypeMasterVcfImport} AND metadata->'chr'->>'value' = '${variant.chr}' AND metadata->'pos'->>'value' = '${variant.pos}' AND metadata->'alt'->>'value' = '${variant.alt}' AND metadata->'ref'->>'value' = '${variant.ref}' ) AS variant ON dtdt_3."data_parentData" = variant.id 
        `.replace(/(?:\r\n|\r|\n)/g, '');

        let sqlquery = sqlfirstPart + sqlsecondPart + sqlthirdPart;
        // logger.log('info', 'NAME: ' + sample.NAME + ' AD: ' + sample.AD + ' - DP: ' + sample.DP + ' - GT: ' + sample.GT + ' - PL: ' + sample.PL)
        // var result = yield knex.raw(sqlquery);
        const result = yield client.query(sqlquery);
    ...
    migrateMasterNGSVCF: async function (folder, ext, processInfo, knex, client) { //, crudManager) {
        var that = this;
        let objInfo = JSON.parse(processInfo.argv[2]);
        ...
        rl.on('line', async (rawLine, index) => {
            ...
            if (workingRows === 0) {
                ...
                var parentData = [];
                var l = '';
                var VariantSupportingInfoArray = [];
                var ExistingVariantSupportingInfo = [];
                ...
                if (variantMasterFile) {
                    try {
                        ...
                        if (parseInt(variantMasterFile.varinfo.AC) <= AlleleCountMaxValue) {
                            // logger.log("info", riga + " " + variantMasterFile.chr + " " + variantMasterFile.pos + " " + variantMasterFile.ref + " " + variantMasterFile.alt + " - count AC: " + variantMasterFile.varinfo.AC);
                            var filteredSamples = variantMasterFile.sampleinfo.filter(s => s.GT !== "./." && s.GT !== "0/0");
                            if (filteredSamples.length > 0) {
                                var start1 = new Date();
                                var results = await that.getPersInfoVariantNGSFromXTENS(filteredSamples, variantMasterFile, null, client);
                                var variant = _.find(results, r => r.idvariant);
                                variantXTENS.id = variant && variant.idvariant;
                                for await (const sample of filteredSamples) {
                                    // get personal variant con figlia variante e padre analisi

                                    var res = _.find(results, res => res.biobank_code === sample.NAME);
                                    var personalInfoID = res && res.suppinfoid;

                                    if (!personalInfoID) {
                                        // GET ANALYSYS FROM TISSUE BIOBANK CODE
                                        var analysisID = await that.getAnalysesNGSFromXTENS(res && res.biobank_code ? res.biobank_code : sample.NAME, daemon, null, client);

                                        if (analysisID.length === 1) {
                                            let metadataPersInfo = utils.composeVCFNGSPersonalInfoMetadata(filteredSamples.find(s => (s.NAME === res && res.biobank_code ? res.biobank_code : sample.NAME)));
                                            ...
                                            let json = [
                                                ...
                                                new Date(),
                                                new Date(),
                                                analysisID.map(d => d.id)
                                            ];

                                            // PUSH JSON INTO ARRAY TO BE CREATED
                                            VariantSupportingInfoArray.push(json);
                                        } else if (analysisID.length === 0) {
                                            logger.log('info', 'No Analysis found for tissue :' + res.biobank_code);
                                        } else {
                                            // PRENDO NOTA DEI TISSUE CHE RISULTANO AVERE PIU ANALISI EXOME

                                        }
                                    } else {
                                        // accantono le supporting variant info gia inserite
                                        ExistingVariantSupportingInfo.push(personalInfoID);
                                    }
                                }

                                // var end = new Date();
                                // var duration = (end.getTime() - start.getTime());
                                // logger.log("info", "ROW: " + riga + " - FilteredSamples Management duration in ms:" + duration);

                                if (VariantSupportingInfoArray.length > 0) {
                                    var parentAnalyses = _.flatten(VariantSupportingInfoArray.map(d => d[d.length - 1]));
                                    var vsiArray = _.map(VariantSupportingInfoArray, d => d.slice(0, -1));
                                    // console.log("INSERT data support", " - ROW: ", riga);

                                    var sl = format('INSERT INTO data (type, owner, metadata, created_at, updated_at) VALUES %L RETURNING id', vsiArray);
                                    var results1 = await client.query(sl, []);

                                    // try {
                                    //     await knex.transaction(async trx => {
                                    // var results1 = await knex('data').returning('id')
                                    //     .insert(VariantSupportingInfoArray)
                                    //     .transacting(trx);
                                    var joinData2 = results1.rows.map((d, i) => {
                                        // return { "data_parentData": d, "data_childrenData": parentAnalyses[i] };
                                        return [d.id, parentAnalyses[i]];
                                    });
                                    // console.log("INSERT data_childrendata__data_parentdata support", " - ROW: ", riga);

                                    const sq = format('INSERT INTO data_childrendata__data_parentdata ("data_parentData", "data_childrenData") VALUES %L', joinData2);
                                    await client.query(sq, []);

                                    // await knex('data_childrendata__data_parentdata')
                                    //     .insert(joinData2)
                                    //     .transacting(trx);
                                    parentData.push(results1.rows.map(d => d.id));
                                ...
                                parentData.push(ExistingVariantSupportingInfo);
                                parentData = _.flatten(parentData);

                                var end1 = new Date();
                                var duration1 = (end1.getTime() - start1.getTime());
                                logger.log("info", "ROW: " + riga + " - IMPORT SVI duration in ms:" + duration1);
                            }
                        }
                        // await client.query('COMMIT');

                        // else {
                        //     // logger.log("info", "TO MUCH LINK");
                        // }
                        ...
                        if (!variantXTENS.id) {
                            variantXTENS.id = await that.getVariantNGSFromXTENS(variantMasterFile, daemon, null, client);
                        }

                        var joinData, joinData1;
                        ...
                        if (variantXTENS.id) {
                            ...
                            variantXTENS.updated_at = new Date();
                            var ColsValues = [
                                variantXTENS.type,
                                variantXTENS.owner,
                                variantXTENS.metadata,
                                variantXTENS.tags,
                                variantXTENS.notes,
                                variantXTENS.updated_at
                            ];
                            ...
                            query.push(set.join(', '));
                            query.push('WHERE id = ' + variantXTENS.id);
                            query.push('RETURNING *');
                            var sqlquery = query.join(' ');
                            // DO UPDATE
                            // console.log("UPDATE data variant", " - ROW: ", riga);

                            // var resUpdate = await client.query(sqlquery, ColsValues);

                            // try {
                            //     await knex.transaction(async trx => {
                            // var resUpdate = await knex('data').returning('id')
                            //     .where({ id: variantXTENS.id })
                            //     .update(variantXTENS)
                            //     .transacting(trx);
                            // joinData1 = parentData.map((p) => {
                            //     // return { "data_parentData": resUpdate.rows[0].id, "data_childrenData": p };
                            //     return [resUpdate.rows[0].id, p];
                            // });
                            // if (joinData1.length > 0) {
                            //     l = format('INSERT INTO data_childrendata__data_parentdata ("data_parentData", "data_childrenData") VALUES %L ON CONFLICT ("data_parentData", "data_childrenData") DO NOTHING', joinData1);

                            //     // console.log("UPDATE data_childrendata__data_parentdata variant", " - ROW: ", riga);

                            //     await client.query(l, []);

                            //     // await knex('data_childrendata__data_parentdata')
                            //     //     .insert(joinData1).onConflict(["data_parentData", "data_childrenData"])
                            //     //     .ignore()
                            //     //     .transacting(trx);
                            // }
                            //     });
                            ...
                            // DO CREATE
                            let json = [[
                                VariantDataTypeMasterVcfImport,
                                45,
                                metadata,
                                null,
                                null,
                            ...
                            // console.log("CREATE data variant", " - ROW: ", riga);
                            var resCreate = await client.query(format('INSERT INTO data (type, owner, metadata, notes, tags, created_at, updated_at) VALUES %L RETURNING id', json), []);
                            // try {
                            //     await knex.transaction(async trx => {
                            // var resCreate = await knex('data').returning('id')
                            //     .insert(json)
                            //     .transacting(trx);
                            joinData = parentData.map((p) => {
                                return [resCreate.rows[0].id, p];
                                // return { "data_parentData": resCreate[0], "data_childrenData": p };
                            });
                            if (joinData.length > 0) {
                                // console.log("data_childrendata__data_parentdata variant", " - ROW: ", riga);
                                await client.query(format('INSERT INTO data_childrendata__data_parentdata ("data_parentData", "data_childrenData") VALUES %L ON CONFLICT ("data_parentData", "data_childrenData") DO NOTHING', joinData), []);

                            //     await knex('data_childrendata__data_parentdata')
                            //         .insert(joinData).onConflict(["data_parentData", "data_childrenData"])
                            //         .ignore()
                            //         .transacting(trx);
                            }
                            //     });
                        ...
                        if (duration2 > 900) {
                            logger.log("info", "ROW: " + riga + " - IMPORT VARIANT duration in ms:" + duration2);
                        }
                        logger.log("info", "ROW: " + riga + " - IMPORT VARIANT duration in ms:" + duration2);
                        // } else {
                        ...
                        // await trx.commit();
                        // logger.log("info", "ROW: " + riga);
                        // console.log(new Date(), " - ROW: ", riga);
                        ...
                        var end = new Date();
                        var duration = (end.getTime() - start.getTime());
                        logger.log("info", "ROW: " + riga + " - IMPORT TOTAL duration in ms:" + duration);
                        // stream.resume();
                        // });
                    } catch (error) {
                        logger.log("error", error);
    ...
    importCNBInfo: BluebirdPromise.coroutine(function * (folder, ext) {
        debugger;
        let files = utils.getFilesInFolder(folder, ext, true);
        ...
        let results = yield BluebirdPromise.each(patients, co.wrap(function * (patient) {
            ...
            let CNBInfo = code ? _.find(CNBInfos, function (obj) {
                return String(obj.metadata['italian_nb_registry_id'].value) === String(code);
            }) : undefined;
    ...
    GetInsertAnalysisNGSforPT: BluebirdPromise.coroutine(function * (patient, daemons) {
        ...
        if (BodyAnalysis && _.isArray(BodyAnalysis) && BodyAnalysis.length > 0) {
            ...
            if (foundAnalyses && foundAnalyses.length > 0 && targetAnalysis === 'WHOLE GENOME') {
                ...
                if (fa !== null && fa !== undefined && targetAnalysis === 'EXOME') {
    ...
    migrateNGSPATIENTS: BluebirdPromise.coroutine(function * (defaultPath, ext, process) {
        // debugger;
        logger.log('info', "NGS PATIENT IMPORT START");
        var that = this;
    ...
    // debugger;
        ...
        if (BodyAnalysis && _.isArray(BodyAnalysis) && BodyAnalysis.length > 0) {
            ...
            logger.log('info', 'if (BodyAnalysis && _.isArray(BodyAnalysis) && BodyAnalysis.length > 0) {');
            ...
            // ho trovato un'analisi con stesso target e stesso target_details --> UPDATE su anWithSameTargetDetails.nested_2_id
                toBeCreated = false;
                logger.log('info', 'anWithSameTargetDetails found');
                ...
                toBeCreated = false;
                logger.log('info', 'anWithSameTargetDetails not found, but set the onlu one analysis ID found: ' + analysis.id);
            }

            // DO UPDATE
            logger.log('info', 'DO UPDATE: ' + analysis.id);
        ...
        logger.log('info', 'DO CREATE: ' + toBeCreated.toString());
        if (toBeCreated) {
            let [resCreaAn, bodyCreaAn] = yield request.postAsync({
                uri: basePath + '/data',
                auth: {
        ...
        yield BluebirdPromise.each(samplesIDs, co.wrap(function * (sampleID, index) {
            ...
            yield BluebirdPromise.each(analysesGroupByType, co.wrap(function * (analysesGrouped, index) {
                ...
                if (reWritePath) {
                    ...
                    logger.log('info', 'DONE  reWritePath: ' + JSON.stringify(resRewrite));
                    if (!resRewrite.analyses) return;
                    analysesGrouped = resRewrite.analyses;
 ...
 */
function Migrator (type, mysqlConn, pgConn, annoPgConn) {
    ...
    // this.knexPgAsync = BluebirdPromise.promisifyAll(require('knex')(connections['pgnode']));
    // this.knexPgAsync = null;
    this.poolpg = null;
    if (type && type === "importVCFMASTER-NGS") {
        const { Pool } = require("pg");
        this.poolpg = new Pool(connections['pgnode']);
    }
...
Migrator.prototype = {
    ...
    migrateMasterNGSVCF: function (folder, ext, process) {
        // let knexAnno = this.knexAnnoPg;
        return coroutines.migrateMasterNGSVCF(folder, ext, process, this.knexPgAsync, this.poolpg) //, this.crudManager)
            .catch(function (err) {
                throw err;
            });
    },
