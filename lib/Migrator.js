...
Migrator.prototype = {
    ...
    migrateSamples: function(mysqlSubjId) {
        ...
        var pgSubjId = that.subjectMap[mysqlSubjId];
    ...
    createPrimary: function(sample, idSubj) {
        ...
        var that = this;
        var knexPg = this.knexPg;
        var sampleType, idPrimarySample;
        var metadata = {
        ...
        return knexPg.select('id').from('data_type').where('name', sampleType)
        ...
        .then(function(ids) { 
            ...
            return that.migratePlasmaSamples(idPrimarySample, idSubj, sampleType);
        })
        
        .then(function() {
            return that.migrateNucleicDerivatives(sample.ID_SAMPLE, idPrimarySample, idSubj);
        });
     ...
     */
    migratePlasmaSamples: function(idPrimarySample, idSubj, sampleType) {
        ...
        .leftJoin('BIT_TISSUE_NAME', 'BIT_TISSUE_NAME.ID_BIT_TISSUE_NAME', 'SAMPLE.ID_BIT_TISSUE_NAME')
        .from("SAMPLE")
        .where('ID_PARENT_SAMPLE', idPrimarySample).andWhere('DESCR_BIT_TISSUE_NAME','PLASMA')

        .then(function(rows) {
            console.log("Migrator.migratePlasmaSamples: got these samples: " + rows);

            return BluebirdPromise.each(function(record) {
                 var metadata = {
                    arrival_date_mb: {value: sample.BM_ARRIVAL_DATE},
                    sample_name: {value: sample.SAMPLE_NAME},
                    sample_codification: {value: sample.DESCR_BIT_TISSUE_NAME},
                 ...
                 return that.knexPg.returning('id').insert({
                    ...
                    'biobank': 1,
                    'metadata': metadata,
                    'biobank_code': record.BIT_CODE,
                    'parent_subject': idSubj,
                    'parent_sample': idParentSample,
                    'created_at': new Date(),
                    'updated_at': new Date()
