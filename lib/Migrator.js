...
const coroutines = {
    ...
    migrateVCF: BluebirdPromise.coroutine(function* (folder, ext, objInfoString) {
        ...
        yield BluebirdPromise.each(files,co.wrap( function *(file) {
            let subject, singleRows = [];
            let fileName = file.split('tmp/')[1], fileNameSplitted = fileName.split('_'), codePatient = fileNameSplitted[0], lab = fileNameSplitted[1], machine = fileNameSplitted[2], capture = fileNameSplitted[3].split('.vcf')[0];
            ...
            if (resSubjects.statusCode !== OK || !bodySubjects) {
                ...
                return BluebirdPromise.rejected("Error Getting Subject");
            }
            if (!bodySubjects.data[0]) {
                [resSubjects, bodySubjects] = yield request.postAsync({
                    uri: basePath + '/subject',
                    auth: { bearer: objInfo.bearerToken },
                    json: {
                        type: objInfo.parentSubjectDtId,
                        metadata:{},
                        owner: objInfo.owner,
                        sex: "N.A.",
                        code: codePatient,
                ...
                subject = bodySubjects[0];
            }
            else {
                subject = bodySubjects && bodySubjects.data[0];
            }

            let results = yield new Promise(function (resolve, reject) {
                ...
                vcf.on('data', function (row){
                    if(row.sampleinfo[0] && subject && subject.id){
                        ...
                        let json = {
                            ...
                            type: parseInt(objInfo.dataTypeId),
                            metadata: metadataVcf,
                            parentSubject: subject.id
                        };
                        singleRows.push(json);
                    }
                    else {
        ...
        yield BluebirdPromise.each(data, co.wrap( function *(datum) {
