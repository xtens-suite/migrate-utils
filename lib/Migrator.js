...
const coroutines = {
    ...
    migrateCGH: BluebirdPromise.coroutine(function* (folder, ext, objInfoString) {
        ...
        let notInserted = 0, created = 0, data = [];
    ...
    migrateVCF: BluebirdPromise.coroutine(function* (folder, ext, objInfoString, knexAnno) {
        // var start = new Date();
        const vcf = require('bionode-vcf');

        let objInfo = JSON.parse(objInfoString);
        let notInserted = 0, created = 0;
...
// CERCO IL TIPO DI DATO VARIANTI PER IL PROGETTO SPECIFICO
            ...
            yield BluebirdPromise.each( variants, co.wrap( function *(datum) {
                ...
                if (res.statusCode !== CREATED) {
                    notInserted += 1;
                    console.log("Datum Not Inserted: ", notInserted);
                }
                else {
                    created = created + 1;
                    // let now = new Date();
...
Migrator.prototype = {
    ...
    migrateVCF: function(folder, ext, objInfo) {
        ...
        return coroutines.migrateVCF(folder, ext, objInfo, knexAnno)
        .catch(function(err) {
            throw err;
        });
    },
