...
const coroutines = {
    ...
    migrateCGH: BluebirdPromise.coroutine(function* (folder, ext, processInfo) {
        ...
        let notInserted = [], created = 0, data = [];
        let fileNames = [];

        let files = utils.getFilesInFolder(folder, ext);
        for (let file of files) {
            fileNames.push(file.split(DEFAULT_LOCATION_TO_SPLIT)[1]);
        }
        let daemon = yield DaemonService.InitializeDeamon(fileNames.length > 0 ? fileNames.join(' -- ') : " ", {}, objInfo.owner, processInfo, objInfo.bearerToken);
        if (files.length === 0){
        ...
        yield BluebirdPromise.each(files,co.wrap( function *(file) {
            logger.log('info',"Migrator.migrateCGH - file: ",file);
            let metadataBatch = utils.composeCGHMetadata(file);

            daemon.info.totalRows = metadataBatch.cnletr.length;
            daemon.info.processedRows = 0;
            daemon = yield DaemonService.UpdateDeamon(daemon, objInfo.bearerToken);
            ...
            if (res.statusCode !== OK || !body) {
                ...
                daemon = yield DaemonService.ErrorDeamon( daemon, res.body, objInfo.bearerToken);

                return BluebirdPromise.rejected(res.body);
            }
            if (!body.data[0]) {
                let errorString = "Migrator.migrateCGH: no sample found";
                daemon = yield DaemonService.ErrorDeamon( daemon, errorString, objInfo.bearerToken);
                return BluebirdPromise.rejected(errorString + ":");
            }
            ...
            if (resSample.statusCode !== OK || !bodyS) {
                ...
                daemon = yield DaemonService.ErrorDeamon( daemon, resSample.body, objInfo.bearerToken);
                return BluebirdPromise.rejected(resSample.body);
            }
            ...
            if (resCGHRaw.statusCode !== CREATED) {
                ...
                daemon = yield DaemonService.ErrorDeamon( daemon, resCGHRaw.body, objInfo.bearerToken);
                return BluebirdPromise.rejected(resCGHRaw.body);
            }
            ...
            if (resCGHProc.statusCode !== CREATED) {
                ...
                daemon = yield DaemonService.ErrorDeamon( daemon, resCGHProc.body, objInfo.bearerToken);
                return BluebirdPromise.rejected(resCGHProc.body);
            }

            logger.log('info', 'Created CGH-Processed: ' + bodyCGHProc);
            ...
            if (resGenProf.statusCode !== CREATED) {
                ...
                daemon = yield DaemonService.ErrorDeamon( daemon, resGenProf.body, objInfo.bearerToken);
                return BluebirdPromise.rejected(resGenProf.body);
            }

            logger.log('info', 'Created GENOMIC PROFILE: ' + bodyGenProf);
            ...
            yield BluebirdPromise.each(metadataBatch.cnletr, co.wrap( function *(cnv, index) {
                ...
                if (res.statusCode !== CREATED) {
                    ...
                    created = created + 1;
                    daemon.info.processedRows = created;
                    daemon = yield DaemonService.UpdateDeamon(daemon, objInfo.bearerToken);
                }
            }));
        ...
        daemon.info.processedRows = created;
        daemon.info.notProcessedRows = notInserted;
        daemon = yield DaemonService.SuccessDeamon(daemon, objInfo.bearerToken);

        logger.log("info","All CGH files were stored correctly");
        return {created: created, notUpdated: notInserted};
    }),
    ...
    migrateVCF: BluebirdPromise.coroutine(function* (folder, ext, processInfo, knexAnno) {
        ...
        yield BluebirdPromise.each(files,co.wrap( function *(file) {
            ...
            let subject, tissue, rowsFilesReady = [];
            let fileName = file.split('tmp/')[1],
                fileNameSplitted = fileName.split('_'),
                codePatient = fileNameSplitted[0],
                idFamily = fileNameSplitted[1],
                tissueID = fileNameSplitted[2].split('-'),
                tissueType = tissueID[0],
                tissueCode = tissueID[1],
                machine = fileNameSplitted[3],
                capture = fileNameSplitted[4].split('.vcf')[0];
            // CERCO IL SOGGETTO
            let queryPayload = {
                "isStream":false,
                "queryArgs":{
...
// CONTROLLO CHE IL FILE NON SIA GIÀ STATO INSERITO
            let filesAlreadyInserted = subject.metadata && subject.metadata.data_files_inserted ? subject.metadata.data_files_inserted.values : [];
...
// CERCO IL TESSUTO
            ...
            let biobankCode = codePatient ? codePatient + '-' + tissueCode : objInfo.idProject + '-' + tissueCode;
...
// SE IL TESSUTO NON ESISTE LO CREO
            if (!bodyTissues.data[0]) {
                ...
                if (resTissueCr.statusCode !== CREATED) {
                    daemon = yield DaemonService.ErrorDeamon( daemon, bodyTissueCr, objInfo.bearerToken);
                    logger.log('error', "Tissue was not correctly created");
                    return BluebirdPromise.rejected(bodyTissueCr);
                }
...
// CERCO IL TIPO DI DATO ANALISI PER IL PROGETTO SPECIFICO
            ...
            if (!analysisDataType[0] || !analysisDataType[0].id) {
                ...
                daemon = yield DaemonService.ErrorDeamon( daemon, resAnalysisDt.body, objInfo.bearerToken);
                return BluebirdPromise.rejected(resAnalysisDt.body);
            }
...
// CREO L'ANALISI
            ...
            if (resAnalysisCr.statusCode !== CREATED) {
                ...
                daemon = yield DaemonService.ErrorDeamon( daemon, resAnalysisCr.body, objInfo.bearerToken);
                return BluebirdPromise.rejected(resAnalysisCr.body);
            }
...
// CERCO IL TIPO DI DATO VARIANTI PER IL PROGETTO SPECIFICO
            ...
            if (!variantDataType[0] || !variantDataType[0].id) {
                ...
                daemon = yield DaemonService.ErrorDeamon( daemon, resVariantDt.body, objInfo.bearerToken);
                return BluebirdPromise.rejected(resVariantDt.body);
            }

            // CREO TUTTE LE VARIANTI
            // let last = new Date(), total = 0;
            daemon.info.totalRows = variants.length;
            daemon.info.processedRows = created;
            daemon = yield DaemonService.UpdateDeamon(daemon, objInfo.bearerToken);
...
//AGGIORNO IL SOGGETTO AGGIUNGENDO IL NOME DEL FILE APPENA CARICATO
            ...
            if (!subjectToUpdate || !subjectToUpdate.id) {
                ...
                daemon = yield DaemonService.ErrorDeamon( daemon, resSubjectGet.body, objInfo.bearerToken);
                return BluebirdPromise.rejected(resSubjectGet.body);
            }
        ...
        logger.log("info","All VCF files were stored correctly");
