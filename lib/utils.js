...
function getFilesInFolder(folder, ext, isDeep) {
    let res = [];
    folder = process.env.NGSPrefixDebugPath + folder;
...
function* RewritePathNGS(analyses, daemon) {
    ...
    yield BluebirdPromise.each(analyses, co.wrap(function* (an) {
        ...
        if (!fs.existsSync(newPathFq1)) {
            ...
            logger.log('info', 'RewritePathNGS -  newPathFq1 created');
            } catch (error) {
            ...
            logger.log('info', 'RewritePathNGS -  newPathFq1 already created: skipped');
        }
        ...
        if (!fs.existsSync(newPathFq2)) {
            ...
            logger.log('info', 'RewritePathNGS -  newPathFq2 already created: skipped');
        }
