 ...
 */
 ...
 function composeCNBInfoMetadata(patient) {
     ...
     let dgnDate = moment(patient['DATA_DG']);
    //  let altraDate = moment(patient['DATA_DG']);
    //  console.log(dgnDate.format('YYYY-MM-DD'),altraDate.format('YYYY-MM-DD'));
     let relDate = patient['DATA_RECIDIVAPM'] ? moment(patient['DATA_RECIDIVAPM']) : null;

     let FuDate = moment(patient['DATA_FU_CLINICO']);
     ...
     case "LNESG2":
         ...
         break;
     default:
         protocol = patient['D_NOME_PROT'].toUpperCase();
 ...
 function composeVCFMetadata(datum, machine, capture) {
     ...
     let chr, pos, id, qual, ref, alt, filter, gt, gtAllele, pl, gq, dp, ad, sangerValid, pathological, varsomeLink;
     chr = datum.chr && datum.chr;
     ...
     ad = datum.AD ? parseInt(datum.AD) : datum.sampleinfo[0] &&  datum.sampleinfo[0].AD ? parseInt(datum.sampleinfo[0].AD) : datum.varinfo.AD ? parseInt(datum.varinfo.AD ): null;
