...
function composeCNBInfoMetadata (patient) {
    ...
    let dgnDate = moment(patient['DATA_DG'], "DD/MM/YYYY");
    //  let altraDate = moment(patient['DATA_DG']);
    //  console.log(dgnDate.format('YYYY-MM-DD'),altraDate.format('YYYY-MM-DD'));
    let relDate = patient['DATA_RECIDIVAPM'] ? moment(patient['DATA_RECIDIVAPM'], "DD/MM/YYYY") : null;

    let FuDate = patient['DATA_FU_CLINICO'] ? moment(patient['DATA_FU_CLINICO'], "DD/MM/YYYY") : null;
    ...
    yield BluebirdPromise.each(analyses, co.wrap(function * (an) {
        ...
        if (!fs.existsSync(newFilePath1)) {
            try {
                ...
                fs.chmodSync(newFilePath1, fs.constants.O_RDONLY);
