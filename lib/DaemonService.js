...
const daemonDisabled = connections.daemonDisabled ? connections.daemonDisabled : false;
...
const coroutines = {
    ...
    InitializeDeamon: BluebirdPromise.coroutine(function * (source, infoObj, operator, processInfo, token) {
        ...
        logger.log('info', 'InitializeDeamon START');
        let [res, existingDaemon] = yield request.getAsync({
            uri: basePath + '/daemon?source=' + source,
            auth: {
                bearer: token
            }
        });
        logger.log('info', 'InitializeDeamon CALL EXECUTED, response :' + existingDaemon);

        existingDaemon = JSON.parse(existingDaemon);
        let alreadyProcessing = false;
        for (let dmn of existingDaemon) {
            if (dmn.status === RUNNING || dmn.status === INITIALIAZING) {
        ...
        let info = {
            ...
            error: alreadyProcessing ? source + " file is already running" : ""
        };
        let daemon = {
            pid: processInfo.pid,
            source: source,
            operator: operator,
            info: info,
            status: alreadyProcessing ? ERROR : INITIALIAZING,
            createdAt: new Date(),
            updatedAt: new Date()
        };
    ...
    ErrorDeamon: BluebirdPromise.coroutine(function * (daemon, error, token) {
        daemon.info.error = error || new Error("Get some error during customised data Import" + daemon.source ? ": " + daemon.source : "");
        daemon.status = ERROR;
        daemon.updatedAt = new Date();
